...
class EmploymentContractTaxCalculation extends \IOProject\Accountancy\ContractSalary\ContractTaxCalculation {
    ...
    public function calculateGrossToNet($grossSalary) {
        ...
        $incomeCost = new Tax(111.25, TaxValueType::Constant, 'Koszt uzyskania przychodu');
        $incomeCost->cal();
        ...
        $taxRelief = new Tax(46.33, TaxValueType::Constant, 'Ulga podatkowa');
        $taxRelief->cal();

        $advance = round($this->aTax['income-tax']->getOutcome() - $taxRelief->getOutcome() - $this->aTax['health-insurance-deduction']->getOutcome());
